...
The most fundamental approach to smoothing volatile data, extracting underlying signals, and establishing a baseline forecast is the Moving Average. The application implements the Simple Moving Average, while the Weighted Moving Average is considered as a theoretical extension to accommodate different temporal behaviors.
...
Although not implemented in the final application, the Weighted Moving Average (WMA) represents a direct theoretical evolution and improvement over the SMA by mathematically mitigating the lagging effect. The WMA assigns predefined, decreasing weights to historical data points, ensuring recent data heavily influences the forecast while distant data decays. Let $w_i$ represent the weight applied to the observation $i$ periods ago, where the sum of all weights strictly equals 1: $\sum_{i=1}^{k} w_i = 1$. The formula is:
\begin{equation}
...
where:
\begin{itemize}
    \item $\mathcal{C}(\cdot)$ is a predefined cost function measuring the fit of the data within a specific segment. Common choices in the literature include the negative log-likelihood or the residual sum of squares assuming a constant mean within each segment (a mean-shift cost). However, for economic time series that exhibit sustained trends, the implementation in this application uses an Ordinary Least Squares (OLS) residual cost, which fits a linear trend (slope and intercept) within each segment. This detects \textit{structural breaks in trend} rather than simple mean shifts—a critical distinction, since a mean-shift cost would fragment a steadily growing series (such as GDP) into many artificially flat segments, whereas the OLS cost only introduces a break when a new slope fits significantly better (Bai and Perron, 1998).
...
Because this condition guarantees that $\tau$ is permanently suboptimal, the temporal index $\tau$ is pruned from the set of candidate change points. As the algorithm progresses forward through the time series, the list of potential previous change points remains small, resulting in $\mathcal{O}(n)$ linear computational performance. This algorithmic efficiency is what allows the Node.js V8 application architecture to calculate structural breaks natively and synchronously without UI thread starvation, completely bypassing the need for computationally heavy external Python sub-processes.
