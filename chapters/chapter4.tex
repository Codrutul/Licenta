...
\begin{enumerate}
    ...
    \item \textbf{The Analytical Side-Panel Drawer:} A collapsible panel on the right side of the interface. Features three tabs: the \textit{AI Analyst} view, the \textit{Stats} statistics panel, and the raw \textit{Data Table} inspection view.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{thesis_images/ui_design.png}
    \caption{The overall physical architecture of the UI viewport during the earlier stages of development. The dark layout emphasizes the central workspace, while docking configuration panels and statistics inside the right-sided panel.}
...
\node (app) [draw, rectangle, fill=blue!5, minimum width=4.5cm, minimum height=2cm, text width=4.1cm] {
    ...
    - timeSeriesData: TimeSeriesData \\ - sessionStorage: Storage \\
    \rule{\linewidth}{0.4pt}\vspace{0.5mm}
    + handleFileUpload() \\ + handleClearData()
};
...
\node (ai) [draw, rectangle, fill=purple!5, minimum width=3.5cm, minimum height=1.5cm, text width=3.1cm, below right=1.5cm and 0cm of app] {
    ...
    \rule{\linewidth}{0.4pt}\vspace{0.5mm}
    + handleGenerate() \\ + MarkdownRenderer()
};
...
\begin{figure}[H]
    ...
    \caption{The HTML5 Canvas chart displaying a global Moving Average forecast alongside historical actual data and its corresponding 95\% confidence interval bands.}
    \label{fig:recharts_pelts}
\end{figure}
 
As shown in Figure \ref{fig:recharts_pelts}, a consistent color scheme is applied to the Canvas chart. The interface renders the historical actual data in blue, the projected global forecast line in purple, and the fitted values in gray, surrounded by the semi-transparent 95\% confidence interval boundaries. 
...
\node (router) [draw, rectangle, fill=orange!5, minimum width=10cm, minimum height=1.8cm, text width=9.5cm] {
    ...
    \rule{\linewidth}{0.4pt}\vspace{0.5mm}
    \textit{+ post(/calculate)} \qquad \textit{+ post(/changepoints)}
};
...
\begin{figure}[H]
    ...
    \caption{The PELT algorithm identifying significant structural breaks, rendered as yellow indicator lines within high-variance windows.}
...
\draw [dashed] (2,0.3) -- (2,5.0);
\draw [dashed] (4,0.3) -- (4,5.0);

% Activation boxes
\draw [fill=blue!20] (-0.1,1) rectangle (0.1,4.5);
\draw [fill=orange!20] (1.9,1) rectangle (2.1,4.5);
\draw [fill=green!20] (3.9,1.5) rectangle (4.1,3.5);

% Messages
\draw [->, thick] (0.1,1) -- node[above] {POST /api/forecast/changepoints} (1.9,1);
\draw [->, thick] (2.1,1.5) -- node[above] {pelt(data, penalty)} (3.9,1.5);

% Self-call / Internal Loop on Service
\draw [->, thick] (4.1,2.0) -- ++(0.3,0) -- ++(0,0.8) -- node[right, align=left, font=\tiny] {Run PELT Search\\\& Calculate BIC\\Penalty} (4.1,2.8);

\draw [->, thick, dashed] (3.9,3.5) -- node[above] {return \{changePoints\}} (2.1,3.5);
\draw [->, thick, dashed] (1.9,4.5) -- node[above] {HTTP 200 OK} (0.1,4.5);

\end{tikzpicture}
\caption{UML Sequence Diagram demonstrating the strict request lifecycle. The Express API handles the HTTP handshake, executing the PELT algorithm via the imported Service Module, before returning the boundaries and concluding the packet.}
...
\begin{itemize}
    \item \textbf{The Global Overview Payload:} When a user first visualizes a dataset, the system programmatically injects an instruction overriding the AI's default personality: "\textit{You are a helpful data assistant. Write a very simple, 1-paragraph summary of the forecasting results. Just state what the data is doing, how many structural breaks there were, and if a Segmented forecast is provided, which model was more accurate based on RMSE. Conclude with a plain English sentence explaining the final projected value and explicitly evaluate if this result is realistic.}" This restricts the neural network to explaining standard deviations and moving averages, preventing it from generating unsupported market predictions.
    
    \item \textbf{The Structural Breakpoint Payload:} If the natively integrated JavaScript PELT algorithm detects a significant trajectory shift, the AI pipeline is instantly swapped to a targeted prompt: "\textit{You are a critical economic analyst. For EACH break date provided, write exactly ONE bullet point briefly naming the most likely real-world economic event or shock that caused the regime change, and state how the trend shifted.}" This ensures the AI specifically discusses the heightened volatility of the temporal event, rather than summarizing the overall 10-year graph.
    
    \item \textbf{The Segmented Trend Payload:} When the operator interactively slices the data to delete the obsolete pre-shock coordinates, the AI receives its most restrictive instruction: "\textit{You are a skeptical, critical economic analyst. Your job is to rigorously evaluate whether the projected forecast is economically plausible. If a Segmented forecast exists, state which model has lower RMSE. Critically assess the projected landing value using real-world domain knowledge and sanity checks.}" This directs the AI to validate the user's graphical decision quantitatively, providing mathematical confirmation to a non-expert operator that recalculating the data was objectively correct.
\end{itemize}
...
Managed inside the \texttt{Forecast\-Chart} component, the toolbar communicates exclusively via a \texttt{Toolbar\-State} interface. This design ensures that layer visibility changes propagate immediately to the visualization canvas via React's unidirectional state flow without intermediate API calls.
