...
React.js (Version 19) was selected as the foundational user interface library over prominent alternatives such as Angular or Vue.js. The justification for this decision is rooted deeply in how modern browsers handle graphical rendering computations.
...
\node[box, fill=blue!20, font=\sffamily\small\bfseries] {App.tsx (Root Provider)}
    child {node[box] {Data Ingestion \\ \textit{$\langle$FileUpload$\rangle$}}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{thesis_images/final_ui_design.png}
    \caption{The Final Scientific UI Design constructed in React. The interface utilizes a structured dark-themed grid layout with a dedicated Canvas area ready for data ingestion.}
...
\node (forecastService) [draw, rectangle, minimum width=5cm, minimum height=1cm, below left=2cm and 0.5cm of route, fill=blue!5] {\textbf{ForecastService} \\ \textit{+ holtsWinters()}};
\node (peltService) [draw, rectangle, minimum width=5cm, minimum height=1cm, below=2cm of route, fill=orange!5] {\textbf{ChangePointService} \\ \textit{+ pelt()}};
\node (aiService) [draw, rectangle, minimum width=5cm, minimum height=1cm, below right=2cm and 0.5cm of route, fill=green!5] {\textbf{AIService} \\ \textit{+ generateAnalysis()}};
...
pelt(data, penaltyMultiplier = null, minLen = null) {
  ...
  for (let t = 1; t <= n; t++) {
    ...
    // PELT Pruning: discard sub-optimal candidates
    const newCandidates = [];
    for (const s of candidates) {
    ...
    if (t >= minSegLen) newCandidates.push(t);
    candidates = newCandidates;
  }
  return { changePoints: backtrack(cp), cost: F[n] };
}
\end{lstlisting}
\end{minipage}

The V8 engine optimizes the dense double-loop and array indexing, executing the mathematical pruning steps natively inside the same process memory. This design entirely avoids the performance and serialization overhead that would be introduced by serializing the time series payload as JSON strings across external processes or analytical environments. The computed change-point indexes are returned synchronously to the Express HTTP handler.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}p{3.2cm}p{4.8cm}p{7.2cm}@{}}
        ...
        & \texttt{chart.js} & Core HTML5 Canvas charting library providing the underlying rendering engine for visual plots. \\
        & \texttt{chartjs-plugin-zoom} & Enables interactive zoom and pan operations directly on the Cartesian chart view. \\
        ...
        & \texttt{cors} & Enforces Cross-Origin Resource Sharing (CORS) security policies across router boundaries. \\
        & \texttt{dotenv} & Restricts environment variable access by loading secret keys strictly from local configuration files. \\
